...
class Riwayat_transaksi extends CI_Controller{
	...
	function index(){
		...
		if (!empty($pelanggan) || !empty($no) || !empty($tgl_awal) || !empty($tgl_akhir)){
			$x['data']=$this->m_uji_sampel->get_riwayat_transaksi_filter($tgl_awal,$tgl_akhir,$pelanggan,$no);
			$hsl2='';
			$hsl3='';
			$hsl4='';
			$hsl5='';
			if ($pelanggan!=null){
				$hsl2='| Pelanggan = '.$this->m_anggota->get_by_kode($pelanggan)->row()->anggota_nama." ";
			}
			if ($no!=null){
			...
			$x['data']=$this->m_uji_sampel->get_riwayat_transaksi();
			$x['show']='';
		}
		...
		$x['xpelanggan']=$this->m_anggota->dd();
		$x['xno']=$this->m_uji_sampel->dd_no_identifikasi();
		$this->load->view('operator/riwayat_transaksi/v_riwayat',$x);
	}
