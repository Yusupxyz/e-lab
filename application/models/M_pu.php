...
class M_pu extends CI_Model{
	...
	function simpan($nama,$sp,$tarif){
		$hsl=$this->db->query("insert into tbl_parameter_uji(pu_nama,pu_sp_id,pu_tarif) values 
		('$nama','$sp','$tarif')");
		return $hsl;
	}
	...
	function update($id,$nama,$sp,$tarif){
		$hsl=$this->db->query("update tbl_parameter_uji set pu_nama='$nama',pu_sp_id='$sp',pu_tarif='$tarif'
			where pu_id='$id'");
		return $hsl;
	}
	function hapus($kode){
